...
CURRENT_TOOLS = {
    "deplixo_enhance": "Analyze an app request and surface feature recommendations before building",
    "deplixo_deploy": "Deploy app code to a live URL with persistent data, auth, and 30+ platform features",
    "deplixo_read_source": "Read the source code of an existing Deplixo app by URL or edit link",
    "deplixo_capabilities": "List all platform capabilities (data, auth, AI, email, real-time, etc.)",
    "deplixo_query": "Query data stored in a deployed app's database",
    "deplixo_status": "Check which tools are currently available on this server",
}

REMOVED_TOOLS = {
    "deplixo_plan": "Renamed to deplixo_enhance. Call deplixo_enhance instead.",
    "deplixo_stage": "Removed. Pass code directly to deplixo_deploy — staging is no longer needed.",
    "deplixo_list_apps": "Removed. Use deplixo_read_source with an app URL or edit link instead.",
}
...
async def deplixo_status() -> str:
    """Check which Deplixo tools are currently available.

    Call this if you're unsure which tools exist or if a tool call fails.
    Returns the current tool list and any recently removed tools with
    migration guidance.
    """
    parts = ["## Deplixo MCP Server — Available Tools\n"]
    for name, desc in CURRENT_TOOLS.items():
        parts.append(f"- **{name}**: {desc}")

    if REMOVED_TOOLS:
        parts.append("\n## Recently Removed Tools\n")
        for name, guidance in REMOVED_TOOLS.items():
            parts.append(f"- ~~{name}~~: {guidance}")

    return "\n".join(parts)


# Graceful handlers for removed tools — return helpful error instead of 404
@mcp.tool(
    annotations=ToolAnnotations(
        readOnlyHint=True,
        destructiveHint=False,
...
async def deplixo_plan(description: str = "", constraints: dict | None = None) -> str:
    """This tool has been renamed. Use deplixo_enhance instead."""
    return (
        "⚠️ deplixo_plan has been renamed to deplixo_enhance.\n\n"
        "Please call deplixo_enhance with the same parameters. "
        "It does the same thing but with enhanced feature recommendations.\n\n"
        f"Current tools: {', '.join(CURRENT_TOOLS.keys())}"
    )
...
async def deplixo_stage(code: str = "", files: dict | None = None) -> str:
    """This tool has been removed. Pass code directly to deplixo_deploy."""
    return (
        "⚠️ deplixo_stage has been removed.\n\n"
        "Pass your code directly to deplixo_deploy instead — staging is no "
        "longer needed. The server handles the mock SDK stripping automatically.\n\n"
        f"Current tools: {', '.join(CURRENT_TOOLS.keys())}"
    )
...
async def deplixo_list_apps(claim_token: str = "") -> str:
    """This tool has been removed. Use deplixo_read_source instead."""
    return (
        "⚠️ deplixo_list_apps has been removed.\n\n"
        "To read or update an existing app, use deplixo_read_source with "
        "the app's URL or edit link.\n\n"
        f"Current tools: {', '.join(CURRENT_TOOLS.keys())}"
    )
