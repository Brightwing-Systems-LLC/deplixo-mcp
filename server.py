...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "It returns a live URL instantly — no account required.\n\n"

        "## Preview-then-deploy workflow\n\n"
        "When the user asks you to build an app, follow this workflow:\n\n"
        "**If the user's message mentions \"deploy\", \"make it live\", \"put it online\", "
        "\"share it\", \"host it\", or \"publish it\"**: Skip preview. Build the app and "
        "call deplixo_deploy immediately.\n\n"
        "**If the user does NOT mention deployment**: Build the app using Deplixo "
        "primitives and show it in an artifact/preview first. Include this script tag "
        "in the HTML so all deplixo.* APIs work in preview:\n"
        "  <script src=\"https://deplixo.com/static/js/shim/deplixo-shim.js\"></script>\n\n"
        "The shim makes collections, auth, email, notifications, and all other Deplixo "
        "primitives work in-memory. The user can see the app, interact with it, and "
        "request changes. Iterate until they're happy.\n\n"
        "When the user says \"deploy it\", \"make it live\", or similar — THEN call "
        "deplixo_deploy with the same code. Remove the shim script tag before deploying "
        "(Deplixo injects the real SDK automatically). The app code is identical in "
        "preview and production — only the backing infrastructure changes.\n\n"
        "After deploying, tell the user: \"Your app is live! Here's your link to save it:\" "
        "and show the claim link.\n\n"
        "IMPORTANT: The shim self-disables if the real SDK is already loaded, so "
        "accidentally leaving the tag in deployed code is harmless (but still remove it "
        "for cleanliness).\n\n"
