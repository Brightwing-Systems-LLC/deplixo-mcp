...
_SDK_SNIPPETS = {
    ...
    "deplixo.rooms": (
        ...
        "  room.presence.join({ name: playerName });\n"
...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "  await recipes.addIfEmpty({ phase: \"waiting\" })        -> atomic create-if-not-exists\n"
        "    Returns { id, value, author, created: true } if inserted, { ..., created: false } if collection already had data.\n"
        "    Use this to safely initialize shared state without race conditions.\n"
        ...
        "Use rooms for ANY app where 2+ users share a space: games, lobbies, \n"
        "collaborative tools, watch parties. Rooms give you scoped collections, \n"
        "broadcast, and presence — no manual prefixing or state machines needed.\n\n"
        "  const room = deplixo.rooms.join(roomId);       // join or create\n"
        "  const state = room.collection(\"game\", { personal: false }); // room-scoped\n"
        "  room.broadcast.send(\"move\", { x: 1, y: 2 });  // ephemeral signals\n"
        "  room.broadcast.on(\"move\", (data) => { });      // listen\n"
        "  room.presence.join({ name: playerName });       // who's in the room\n"
        "  room.presence.onChange(({ action }) => {\n"
        "    room.presence.list().then(users => updatePlayerList(users));\n"
        "  });\n"
        "  room.host      // userId of the host (alphabetically first player)\n"
        "  room.isHost    // true if current visitor is the host\n"
        "  room.onReady(minPlayers, callback)  // fires ONCE on HOST only when enough players join\n"
        "  room.leave();                                   // clean up\n"
        "  const rooms = await deplixo.rooms.list();       // list all rooms\n"
        "  const newRoom = await deplixo.rooms.create({ name: \"Game Room\" });\n\n"
        "IMPORTANT: room.collection() auto-prefixes the collection name with the room ID.\n"
        "This means each room has its own isolated data — no manual namespacing needed.\n"
        "ALWAYS use room.collection() instead of deplixo.db.collection() inside rooms.\n\n"
        "### Timers (server-side countdown/stopwatch)\n"
        "  await deplixo.timers.start(\"round\", 30000);   // 30s countdown\n"
        "  await deplixo.timers.pause(\"round\");\n"
        "  await deplixo.timers.resume(\"round\");\n"
        "  const s = await deplixo.timers.status(\"round\"); // { state, remaining_ms }\n"
        "  deplixo.timers.onExpire(\"round\", () => endRound());\n"
        "  deplixo.timers.clear(\"round\");\n"
        "Server-side — survives page refresh. Use for game rounds, quiz timers, etc.\n\n"
        "### Multiplayer Game Pattern (rooms + presence + broadcast + timers)\n"
        "Use this pattern for turn-based or real-time multiplayer games:\n\n"
        "  // 1. Join a room (both players enter the same roomId)\n"
        "  const room = deplixo.rooms.join(roomId);\n"
        "  const gameState = room.collection(\"state\", { personal: false });\n\n"
        "  // 2. Register onChange FIRST — before any .list() or .add() calls\n"
        "  //    Events fired before registration are lost.\n"
        "  gameState.onChange(({ action, id, value }) => render(value));\n\n"
        "  // 3. Join presence and use onReady to initialize state safely\n"
        "  room.presence.join({ name: playerName });\n"
        "  room.onReady(2, (players) => {\n"
        "    // Fires on HOST only — no race condition\n"
        "    gameState.add({ phase: \"playing\", round: 1,\n"
        "      setterId: players[0].userId, guesserId: players[1].userId,\n"
        "      scores: {} });\n"
        "  });\n\n"
        "  // 4. Use broadcast for ephemeral events (animations, sound cues)\n"
        "  room.broadcast.send(\"reaction\", { emoji: \"🎉\" });\n"
        "  room.broadcast.on(\"reaction\", (data) => showEmoji(data.emoji));\n\n"
        "  // 5. Optional: timed rounds\n"
        "  await deplixo.timers.start(\"round\", 30000);\n"
        "  deplixo.timers.onExpire(\"round\", () => {\n"
        "    deplixo.sound.play(\"@ding\");\n"
        "    endRound();\n"
        "  });\n\n"
        "Key rules for multiplayer games:\n"
        "- ALWAYS use room.onReady() to initialize game state — it fires on ONE client only (the host)\n"
        "- ALWAYS register onChange() BEFORE calling .list() or .add() to avoid missing events\n"
        "- ALWAYS use room.collection() for game state, not deplixo.db.collection()\n"
        "- Store ALL game state (scores, turns, board) in the room collection — "
        "not in JS variables\n"
        "- Use onChange() to re-render on ALL clients when state changes\n"
        "- Use broadcast for ephemeral signals only (typing indicators, animations)\n"
        "- Alternatively, use gameState.addIfEmpty(initialState) for atomic create-if-not-exists\n\n"
        ...
        "- NEVER build multiplayer rooms/lobbies with raw deplixo.db.collection() and manual name prefixing — use deplixo.rooms.join(roomId) which gives room-scoped collections, broadcast, and presence automatically\n"
        "- NEVER have multiple clients race to create initial game state — use room.onReady(n, cb) (host-only callback) or collection.addIfEmpty() (atomic server-side)\n"
        "- ALWAYS register .onChange() BEFORE calling .list() or .add() — events fired before registration are lost\n"
